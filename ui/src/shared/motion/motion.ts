...
/** Neutral skin source: the composition root injects the live app skin here. */
export type MotionProfileProvider = () => MotionSkinId;

let profileProvider: MotionProfileProvider = () => 'aurora';

/** Register the live skin provider (main.ts wires the app skin to this port). */
export function configureMotionProfileProvider(provider: MotionProfileProvider): void {
  profileProvider = provider;
}

/** Test-only reset: restores the default-skin provider between tests. */
export function resetMotionProfileProviderForTests(): void {
  profileProvider = () => 'aurora';
}

function currentProfile() {
  return getMotionProfile(profileProvider());
}
