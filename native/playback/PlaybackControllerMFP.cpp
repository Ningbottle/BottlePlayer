...
class MediaPlayerCallback final : public IMFPMediaPlayerCallback {
 public:
  explicit MediaPlayerCallback(PlaybackControllerMFP* owner) : owner_(owner) {}
 ...
 private:
  ...
  std::atomic<PlaybackControllerMFP*> owner_;
};

PlaybackControllerMFP::~PlaybackControllerMFP() {
  {
...
bool PlaybackControllerMFP::Initialize() {
  std::lock_guard lock(mutex_);
...
bool PlaybackControllerMFP::PlayUrl(const std::string& url) {
  std::lock_guard lock(mutex_);
  if (!mediaFoundationStarted_) {
    state_.kind = echo::core::PlaybackStateKind::Failed;
...
void PlaybackControllerMFP::Pause() {
  std::lock_guard lock(mutex_);
  if (player_) {
    player_->Pause();
  }
...
void PlaybackControllerMFP::Resume() {
  std::lock_guard lock(mutex_);
  if (player_) {
    player_->Play();
  }
...
void PlaybackControllerMFP::Stop() {
  std::lock_guard lock(mutex_);
  ReleasePlayerLocked();
...
void PlaybackControllerMFP::Seek(double seconds) {
  std::lock_guard lock(mutex_);
...
void PlaybackControllerMFP::SetVolume(double volume) {
  std::lock_guard lock(mutex_);
...
void PlaybackControllerMFP::SetRate(double rate) {
  std::lock_guard lock(mutex_);
...
echo::core::PlaybackState PlaybackControllerMFP::GetState() const {
  std::lock_guard lock(mutex_);
...
void PlaybackControllerMFP::ReleasePlayerLocked() {
  if (!player_) {
    return;
  }
  player_->Stop();
...
bool PlaybackControllerMFP::EnsurePlayerLocked() {
  if (player_) {
    return true;
  }
  if (!callback_) {
...
void PlaybackControllerMFP::HandleMediaEvent(MFP_EVENT_HEADER* event) {
  if (!event) {
    return;
  }
